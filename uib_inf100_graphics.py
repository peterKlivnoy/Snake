...
if ((sys.version_info[0] != 3) or (sys.version_info[1] < 6)):
    raise Exception(
        'uib_inf100_graphics.py requires Python version 3.6 or later.')
...
try:
    ...
    failed_import('PIL', 'pillow')

if sys.platform.startswith('linux'):
    try:
        import pyscreenshot as ImageGrabber
    except ModuleNotFoundError:
        failed_import('pyscreenshot')
else:
    try:
...
class WrappedCanvas(Canvas):
    ...
    def log(self, method_name, args, kwargs):
        if (not self.in_redraw_all):
            self.app._mvc_violation(
                'you may not use the canvas (the view) outside of redraw_all')
    ...
    def create_image(self, *args, **kwargs):
        ...
        if ((not uses_image) and (not uses_pil_image)):
            ...
            raise Exception(
                'create_image cannot use both an image and a pil_image')
        elif (uses_pil_image):
            pil_image = kwargs['pil_image']
            del kwargs['pil_image']
            if (not isinstance(pil_image, Image.Image)):
                raise Exception(
                    'create_image: pil_image value is not an instance of a PIL/Pillow image')
...
class App(object):
    ...
    def redraw_all(app, canvas): pass
    ...
    def save_snapshot(app):
        ...
        if (path):
            ...
            if (not path.endswith('.png')):
                path += '.png'
            app._deferred_method_call(
                afterId='save_snapshot', afterDelay=0, afterFn=lambda: app.get_snapshot().save(path))
    ...
    def __setattr__(app, attr, val):
        ...
        if (d.get('running', False) and
            ...
            app._mvc_violation(
                f'you may not change app.{attr} in the model while in redraw_all (the view)')
    ...
    def _redraw_all_wrapper(app):
        if (not app._running):
            return
        if ('deferred_redraw_all' in app._afterIdMap):
        ...
        try:
            ...
            if (hash1 != hash2):
                app._mvc_violation(
                    'you may not change the app state (the model) in redraw_all (the view)')
        finally:
    ...
    def _deferred_method_call(app, afterId, afterDelay, afterFn, replace=False):
        ...
        if ((id is None) or replace):
            ...
            app._afterIdMap[afterId] = app._root.after(
                afterDelay, afterFn_wrapper)

    def _deferred_redraw_all(app):
        app._deferred_method_call(afterId='deferred_redraw_all',
                                  afterDelay=100, afterFn=app._redraw_all_wrapper, replace=True)
    ...
    def _timer_fired_wrapper(app):
        if (not app._running) or (not app._method_is_overridden('timer_fired')):
            return
        if (not app._paused):
            app.timer_fired()
            app._redraw_all_wrapper()
        app._deferred_method_call(afterId='_timer_fired_wrapper',
                                  afterDelay=app.timer_delay, afterFn=app._timer_fired_wrapper)
    ...
    def _mouse_motion_wrapper(app):
        ...
        if ((not app._paused) and
            ...
            if ((app._lastMousePosn != (event.x, event.y)) and
                ...
                if (app._mouse_is_pressed):
                    app.mouse_dragged(event)
                else:
                    app.mouse_moved(event)
                app._lastMousePosn = (event.x, event.y)
                app._redraw_all_wrapper()
        if (mouse_moved_exists or mouse_dragged_exists):
            app._deferred_method_call(
                afterId='mouse_motion_wrapper', afterDelay=app.mouse_movedDelay, afterFn=app._mouse_motion_wrapper)
    ...
    def run(app):
        ...
        if (App._theRoot is None):
            App._theRoot = Tk()
            # when user enters cmd-q, ignore here (handled in key_pressed)
            ...
            App._theRoot.bind(
                "<Button-1>", lambda event: App._theRoot.app._mouse_pressed_wrapper(event))
            App._theRoot.bind(
                "<B1-ButtonRelease>", lambda event: App._theRoot.app._mouse_released_wrapper(event))
            App._theRoot.bind(
                "<KeyPress>", lambda event: App._theRoot.app._key_pressed_wrapper(event))
            App._theRoot.bind(
                "<KeyRelease>", lambda event: App._theRoot.app._key_released_wrapper(event))
            App._theRoot.bind(
                "<Configure>", lambda event: App._theRoot.app._size_changed_wrapper(event))
        else:
...
class TopLevelApp(App):
    ...
    def __init__(app, fnPrefix='', **kwargs):
        ...
        if ((fnPrefix != '') and ('title' not in kwargs)):
            # f"TopLevelApp '{fnPrefix}'"
            kwargs['title'] = f"INF100 '{fnPrefix}'"
...
print(
    f'Loaded uib_inf100_graphics version {App.version} (last updated {App.last_updated})')
